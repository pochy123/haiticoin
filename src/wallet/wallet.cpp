...
// trigger: optional setting to unlock wallet for block staking only;
...
//    return n ? n : nTimeReceived;
    ...
    // trigger: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// trigger: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // trigger: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // trigger: set time
...
//                }
                ...
                // trigger: sub-cent change is moved to fee
...
// trigger: create coin stake transaction
