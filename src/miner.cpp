...
// trigger: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr <CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, bool fMineWitnessTx, CWallet *pwallet, bool *pfPoSCancel) {
    ...
    // trigger: if coinstake available add coinstake tx
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // trigger: there is no point to continue if we failed to create coinstake
    }
...
//        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TriggerMiner: generated block is stale");
    }
...
void PoSMiner(CWallet *pwallet) {
    ...
    RenameThread("trigger-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            // trigger: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake()) {
...
// trigger: stake minter
void MintStake(boost::thread_group &threadGroup) {
    // trigger: mint proof-of-stake blocks in the background
    if (!vpwallets.empty())
...
void static TriggerMiner(const CChainParams &chainparams, void *parg) {
    LogPrintf("TriggerMiner started\n");
    RenameThread("trigger-miner");
    CWallet *pwallet = (CWallet *) parg;
...
//            }
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("Error in TriggerMiner: Keypool ran out, please call keypoolrefill before restarting the mining hread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running TriggerMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//                        LOCK2(cs_main, pwallet->cs_wallet);
                        if (!SignBlock(*pblock, *pwallet)) {
                            LogPrintf("TriggerMiner(): failed to sign PoS block");
                            continue;
                        }
        ...
        LogPrintf("TriggerMiner terminated!\n");
        throw;
    }
    catch (const std::runtime_error &e) {
        LogPrintf("TriggerMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateTrigger(bool fGenerate, int nThreads, const CChainParams &chainparams) {
    ...
    if (!vpwallets.empty()) {
        for (int i = 0; i < nThreads; i++)
            minerThreads->create_thread(boost::bind(&TriggerMiner, boost::cref(chainparams), vpwallets[0]));
    }
}
