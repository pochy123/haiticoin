...
#if defined(NDEBUG)
# error "Trigger cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Trigger Signed Message:\n";
                              ...
                              bool bypass_limits, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        // trigger: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
...
// trigger: it depends on current POW Block Height, not current blockchain Height
...
// trigger: miner's coin stake is rewarded based on coin age spent (coin-days)
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // trigger: we are using arguments that will have no effects in updateAlert()
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // trigger
            undo.nTime = alternate.nTime;           // trigger
        } else {
...
bool TriggerContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    uint256 hashProofOfStake = uint256();
    uint256 targetProofOfStake = uint256();
    // trigger: verify hash target and signature of coinstake tx
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!TriggerContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // trigger: coinbase reward check relocated to CheckBlock()
    ...
    // trigger: track money supply and mint amount info
    ...
    // trigger: fees are not collected by miners as in bitcoin
    // trigger: fees are destroyed to compensate the entire network
    if (gArgs.GetBoolArg("-printcreation", false))
...
bool CheckBlock(const CBlock &block, CValidationState &state, const Consensus::Params &consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature) {
    ...
    // trigger: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.DoS(100, false, REJECT_INVALID, "bad-cs-missing", false, "coinstake in wrong position");

    // trigger: first coinbase output should be empty if proof-of-stake block
    ...
    for (const auto& tx : block.vtx)
    {
        ...
        // trigger: check transaction timestamp
...
#ifdef ENABLE_CHECKPOINTS
    // trigger: check that the block satisfies synchronized checkpoint
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        // trigger: Don't reject in case of old clients. Change our assumption instead.
...
//    // trigger: ask for pending sync-checkpoint if any
//    if (!IsInitialBlockDownload())
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool fCheckPoS)
{
    ...
    // trigger: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // trigger: check PoS
    if (fCheckPoS && !TriggerContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
...
#ifdef ENABLE_CHECKPOINTS
    // trigger: check pending sync-checkpoint
...
#ifdef ENABLE_CHECKPOINTS
    // trigger: if responsible for sync-checkpoint send it
...
#ifdef ENABLE_CHECKPOINTS
    // trigger: load hashSyncCheckpoint
...
bool LoadBlockIndex(const CChainParams &chainparams) {
    ...
    if (needs_init) {
        ...
        fTxIndex = true; // trigger: txindex is always enabled
...
// trigger: total coin age spent in transaction, in the unit of coin-days.
